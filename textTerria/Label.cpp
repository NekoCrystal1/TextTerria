...
Label::Label(Widget* parent, const Vector2& position, const Vector2& size, bool is_handle_scale):
	...
	text_original_height(text_height), original_transform(*m_Transform), change_scale(1.5),is_handle_scale(is_handle_scale)
{
}
...
void Label::on_update()
{
	...
	if (is_handle_scale && msg.message == WM_MOUSEMOVE)
		if (check_point_in_rectangle(Vector2(msg.x, msg.y), original_transform)) {
			...
			this->m_Transform->set_scale(change_scale);
			this->m_Transform->set_position(this->original_transform.get_position() - Vector2(
				this->m_Transform->get_size().x / 2 - this->original_transform.get_size().x / 2, this->m_Transform->get_size().y / 2 - this->original_transform.get_size().y / 2));
			text_height = text_original_height * change_scale.x;
		}
		else {
			if (!check_point_in_rectangle(Vector2(msg.x, msg.y), *m_Transform)) {
				text_render_color = text_original_color;
				*this->m_Transform = original_transform;
...
void Label::on_render() const
{
	...
	outtextxy(this->m_Transform->get_position().x, this->m_Transform->get_position().y + (this->m_Transform->get_size().y - text_height) / 2, _tcsdup(text.c_str()));
}

void Label::refresh_transform()
{
	settextstyle(text_height, 0, _T("宋体"));
	this->m_Transform->set_size_x(textwidth(_tcsdup(text.c_str())) / (m_Transform->get_scale().x ? m_Transform->get_scale().x : 1));
	if (m_Transform->get_size().y < text_height)
		this->m_Transform->set_size_y(text_height);
	this->original_transform = *this->m_Transform;
}
